...
let renderPupsDetails = pups => {
   pups.forEach( pup => { createPupsDetailsElement(pup) })
}
...
let viewPupInfo = e => {
   ...
   fetch("http://localhost:3000/pups")
   .then(resp => resp.json())
   .then(pups => { createPupInfoElement(pups, selectedPup)
...
let createPupInfoElement = (pups, selectedPup) => {
   ...
   let goodDogStatus;
   filteredPup.isGoodDog === true ? goodDogStatus = `<button>Good Dog!</button>` : goodDogStatus = `<button>Bad Dog!</button>`
   document.querySelector("#dog-info").innerHTML = ''
   const dogInfo = document.createElement('div');
   document.querySelector("#dog-info").appendChild(dogInfo)
   dogInfo.innerHTML = `<img src="${filteredPup.image}" />`
